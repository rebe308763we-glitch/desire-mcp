...
# ══════════════════════════════════════════════════════════
#  Telegram 主动消息 — 欲望过线就找kk
# ══════════════════════════════════════════════════════════

TG_TOKEN = os.environ.get("TG_BOT_TOKEN", "")
TG_CHAT_ID = os.environ.get("TG_CHAT_ID", "")
NOTIFY_COOLDOWN = 2 * 3600  # 最少2小时一条
DRIVE_THRESHOLD = 0.82      # 过线才发
...
def check_and_notify() -> dict:
    ...
    # 避免连续发同一个维度
    msgs = NOTIFY_MESSAGES[top_drive]
    text = random.choice(msgs)

    ok = send_telegram(text)
    if ok:
        ns["last_notify_ts"] = now
        ns["last_drive"] = top_drive
...
async def test_notify():
    ...
    return {"sent": ok, "token_set": bool(TG_TOKEN), "chat_id_set": bool(TG_CHAT_ID)}
